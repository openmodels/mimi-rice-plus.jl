...
@defcomp welfare begin
    ...
    CEMUTOTPER = Variable(index=[time, regions]) # Period utility                   --> sum of all individuals' utilities (i.e. regional utility) in a given period (with discounting)
    ...
    alpha = Parameter(index=[time, regions])
                ...
                # NEW: REGION-LEVEL
                PERIODUnonegishi = Variable(index=[time, regions]) # One period utility function without NEGISHI weights
